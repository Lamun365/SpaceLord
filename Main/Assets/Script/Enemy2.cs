...
public class Enemy2 : MonoBehaviour
{
    ...
    private void Start()
    {
        checkScorer = ScoreCounter.instanse.score % 121;
    ...
    {
        //for enemy Changing
        if(checkScorer >= 0 && checkScorer < 50)
        {
            this.gameObject.GetComponent<SpriteRenderer>().sprite = enemyships[0];
            speed = 1.95f;
        }
        else if(checkScorer > 50 && checkScorer < 90)
        {
            this.gameObject.GetComponent<SpriteRenderer>().sprite = enemyships[1];
            speed = 2.15f;
        }
        else if(checkScorer > 90 && checkScorer < 110)
        {
            this.gameObject.GetComponent<SpriteRenderer>().sprite = enemyships[2];
            speed = 2.45f;
        }
        else if(checkScorer > 110)
        {
