...
public class Enemy3 : MonoBehaviour
{
    ...
    private void Start()
    {
        checkScorer = ScoreCounter.instanse.score % 161;
    ...
    {
        //for enemy Changing
        if(checkScorer >= 60 && checkScorer < 80)
        {
            this.gameObject.GetComponent<SpriteRenderer>().sprite = enemyships[0];
            speed = 1.6f;
        }
        else if(checkScorer > 80 && checkScorer < 100)
        {
            this.gameObject.GetComponent<SpriteRenderer>().sprite = enemyships[1];
            speed = 1.8f;
        }
        else if(checkScorer > 100 && checkScorer < 120)
        {
            this.gameObject.GetComponent<SpriteRenderer>().sprite = enemyships[2];
            speed = 1.9f;
        }
        else if(checkScorer > 120 && checkScorer < 140)
        {
            this.gameObject.GetComponent<SpriteRenderer>().sprite = enemyships[2];
            speed = 2.01f;
        }
        else if(checkScorer > 140)
        {
